...
class Dataset:
    def __init__(
        ...
        if isinstance(output_ids, list) and len(output_ids) == 1:
            output_ids = output_ids[0]

        self.output_ids = output_ids

        X, y = self._data[:, self.input_ids], self._data[:, self.output_ids].reshape(
            -1, 1
        )
    ...
    def get_configspace(self):
        ...
        for i, j in enumerate(self.input_ids):
            name = str(self.get_input_labels(j))
            uniform_float_hp = Float(name, (np.min(self.X[:, i]), np.max(self.X[:, i])))
            cs.add_hyperparameter(uniform_float_hp)

        return cs

    def get_data(self, split=0.8, random_state=0):
    ...
    def get_output_labels(self):
        return LABELS[self.dataset_name][self.output_ids]
